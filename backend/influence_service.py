...
class InfluenceGraphBuilder:
    ...
    def build_graph(self, limit: Optional[int] = None) -> nx.Graph:
        """
    ...
    def _add_street_edges(self, citizens: List):
        ...
        for citizen in citizens:
            if citizen.street_id:
                street_groups.setdefault(citizen.street_id, []).append(citizen.id)
    ...
    def _add_booth_edges(self, citizens: List):
        """Add edges for citizens in same booth (sampled to avoid explosion)"""
        booth_groups = {}
        for citizen in citizens:
            booth_groups.setdefault(citizen.booth_id, []).append(citizen.id)

        edge_count = 0
        for booth_id, citizen_ids in booth_groups.items():
            # Connect each citizen to up to 20 random booth‑mates
            for cid in citizen_ids:
                others = [c for c in citizen_ids if c != cid]
                sample_size = min(20, len(others))
                if sample_size > 0:
                    connections = np.random.choice(others, size=sample_size, replace=False)
    ...
    def _add_scheme_edges(self):
        """Add edges for citizens in same government scheme (sampled)"""
        beneficiaries = self.db.query(models.Beneficiary).all()
        scheme_groups = {}
        for ben in beneficiaries:
            scheme_groups.setdefault(ben.scheme_id, []).append(ben.citizen_id)

        edge_count = 0
        for scheme_id, citizen_ids in scheme_groups.items():
            # Connect each citizen to a few others in the same scheme
            for i in range(len(citizen_ids)):
    ...
    def _add_activity_edges(self):
        """Add edges for citizens with shared activities on the same day"""
        activities = self.db.query(models.Activity).limit(5000).all()
        activity_groups = {}
        for act in activities:
            key = f"{act.activity_type}_{act.timestamp.date()}"
            activity_groups.setdefault(key, []).append(act.citizen_id)

        edge_count = 0
        for key, citizen_ids in activity_groups.items():
            if len(citizen_ids) > 1:
                # Connect each citizen to up to 10 others in the same group
                for i in range(len(citizen_ids)):
                    for j in range(i + 1, min(i + 10, len(citizen_ids))):
...
def run_influence_scoring(db: Session, mode: str = 'full', limit: Optional[int] = None) -> Dict:
    """
    ...
    try:
        ...
        # Update booth sentiment cache (optional optimisation)
        ...
        logger.error(f"Influence scoring failed: {e}", exc_info=True)
        return {
            "status": "error",
            "message": str(e),
            "execution_time": round((datetime.now(timezone.utc) - start_time).total_seconds(), 2)
        }


def update_booth_sentiment_cache(db: Session) -> None:
